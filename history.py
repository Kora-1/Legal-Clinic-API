...
async def save_chat(data: ChatRequest):
    ...
    try:
        ...
        # Otherwise: try to reuse last session if within 60 min
...
async def get_user_sessions(username: str):
    try:
        sessions = list(history_collection.find(
            {"username": username},
            # {"_id": 0, "session_id": 1, "started_at": 1, "messages": {"$slice": 1}}  # get first message as preview
            {"_id": 0, "session_id": 1, "started_at": 1, "messages": 1}  # get first message as preview
        ).sort("started_at", -1))
